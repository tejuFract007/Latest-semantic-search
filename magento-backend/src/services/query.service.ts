...
//     const schemaInfo = `
//       Database Schema:
//       - catalog.products (product_id, sku, name, title, brand, part_number, price, in_stock, qty, short_description, description, created_at, updated_at)
//       - catalog.categories (category_id, name, parent_id, level)
//       - catalog.product_categories (links products to categories)
//       - catalog.product_attributes (attribute_code, value_text)
//       - catalog.v_products_safe (safe view with aggregated categories)

//       Rules:
//       - Only use SELECT queries
//       - Always include LIMIT 50 if not specified
//       - Use ILIKE for text searches
//       - Use to_tsvector for full-text search
//       - Return only from safe views when possible
//     `;
...
//     // Search filter - FIX THIS PART TOO
//     if (filters.search) {
//       paramCount++;
//       query += ` AND (
